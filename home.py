...
class siswa:
    def __init__(self, root):
        ...
        # Define your GUI components here (entry widgets, treeview, etc.)

        # Example: Entry for Id_Siswa
        self.Id_SiswaEntry = tk.Entry(self.root)
        self.Id_SiswaEntry.pack()

        # Example: Treeview
        self.my_tree = tk.ttk.Treeview(self.root)
        self.my_tree.pack()

        # Create necessary buttons (e.g., "Tambah", "Ubah", "Hapus")
        self.tambah_button = tk.Button(self.root, text="Tambah", command=self.tambah)
        self.tambah_button.pack()

        self.ubah_button = tk.Button(self.root, text="Ubah", command=self.ubah)
        self.ubah_button.pack()

        self.hapus_button = tk.Button(self.root, text="Hapus", command=self.hapus)
        self.hapus_button.pack()
        
        self.hapus_button = tk.Button(self.root, text="Hapussemua", command=self.hapussemua)
        self.hapus_button.pack()
        
        self.hapus_button = tk.Button(self.root, text="pilih", command=self.pilih)
        self.hapus_button.pack()
        
        self.hapus_button = tk.Button(self.root, text="cari", command=self.cari)
        self.hapus_button.pack()

#koneksi untuk phpmyadmin
def connection():
    conn = pymysql.connect(
        host='localhost', user='root', password='', db='siswadb'
    )
    return conn
